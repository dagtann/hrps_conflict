# to create file: figure3_2_04_csv
# event only


# set time start variable
systime1 <- Sys.time()
print(Sys.time() - systime1)

# load libraries
library(rjags); library(coda); library(foreign)

STARTYEAR <- 1949
#STARTYEAR <- 1956

data <- read.csv("FarissAPSRData1949_2010_prepared_for_analysis_04.csv")
data <- subset(data, data$YEAR >= STARTYEAR)
data <- subset(data, select=c(YEAR, CIRI, COW, DISAP, KILL, POLPRIS, TORT, Amnesty, State, hathaway, ITT, genocide, rummel, massive_repression, executions, killing))


data <- subset(data, select=c(YEAR, CIRI, COW, genocide, rummel, massive_repression, executions, killing))

nrow(data)
n <- nrow(data)


year <- NA
year[1] <- 1
country <- NA
panel <- NA
panel.count <-1
i <- 2
country[1] <- 1
j <- 1
while(i <= nrow(data)){
if(data$COW[i]!=data$COW[i-1]){
panel[j] <- panel.count
panel.count <- 0
j <- j+1
}
country[i] <- j
#i <- i + 1
panel.count <- panel.count + 1
year[i] <- panel.count
i <- i + 1
}
j
panel[j] <- panel.count

# full model (13 items)
y.pre <- as.matrix(data[,4:8])
y <- matrix(data=NA, ncol=ncol(y.pre), nrow=nrow(y.pre))
for(i in 1:nrow(y.pre)){
  for(j in 1:5){
    y[i, j] <- as.numeric(y.pre[i, j]) 
  }
  }


head(y)
time <- data$YEAR - STARTYEAR + 1


# random initial values
MakeInits <- function(){
  nIRT.Binary <- 5
    
  MU <- matrix(rnorm(length(panel)*max(panel), mean = 0, sd = 1), nrow=length(panel),  ncol=max(panel))

  BETA1 <- runif(nIRT.Binary)
  ALPHA1 <- runif(nIRT.Binary)

    
  SIGMA <- runif(1)

  out <- list(mu=MU, alpha1=ALPHA1, beta1=BETA1,  sigma=SIGMA)
  return(out)
}
inits1 <- MakeInits()
inits2 <- MakeInits()
inits3 <- MakeInits()

inits.function <- function(chain){
  return(switch(chain,
         "1"=inits1,
         "2"=inits2,
         "3"=inits3
         ))
}

# jags.model arguments
ADAPT <- 1000
BURNIN <- 15000
DRAWS <- 150000
THIN <- 10
CHAINS <- 2

NAME <- "EventOnlyDynamicDynamic.bug"

# print time taken
print(Sys.time() - systime1)
  
#rjags code version
m <- jags.model(file=NAME, data=list("y"=y,  "year"=year, "country"=country, "n.country"=length(panel), "n.year"=max(panel), "n"=nrow(y)), inits=inits.function, n.chains=CHAINS, n.adapt=ADAPT)

print(Sys.time() - systime1)

update(m, BURNIN)
print(Sys.time() - systime1)


systime1 <- Sys.time()

# j <- dic.samples(m, n.iter=DRAWS, thin=THIN, type="pD")
# j

save.image(file="image.Rdata")



M <- coda.samples(m, variable.names=c("x"), n.iter=DRAWS, progress.bar="text", thin=THIN)




print(Sys.time() - systime1)



mat1 <- as.matrix(as.mcmc(M[[1]]))
mat2 <- as.matrix(as.mcmc(M[[2]]))
posterior.estimates <- rbind(mat1, mat2)




vars <- t(as.matrix(posterior.estimates))
parameter.mean <- apply(vars, 1, mean)
scoret <- as.matrix(parameter.mean)

ally.pre <- cbind(y.pre, scoret)
all.data <- cbind(data, ally.pre)
write.csv(as.data.frame(all.data), "figure3_2_04.csv", row.names=F)

