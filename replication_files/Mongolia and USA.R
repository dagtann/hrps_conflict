...
dataMongol <- subset(data1,  country == "Mongolia" & YEAR>=1949 & YEAR<=2010)
...
draws <- matrix(NA, nrow=5000, ncol=length(year))
...
for(i in 1:length(year)){
  draws[,i]  <- rnorm(5000, mean=latentmean[i], sd=latentsd[i])
}
