# to create figure5_04_csv
# all inputs have variable intercepts
...
# this will use data where CIRI is replacing with random variable
...
#here we use all Indicators as in APSR but run dynamic model for the Event Indicators also
...
MakeInits <- function(){
 nIRT.Binary <- 5; Cuts1 <- 1
  ...
  ALPHA01 <- array(c(runif( nIRT.Binary, 0, 1)),
                    dim=c( nIRT.Binary, Cuts1, max(year)))
  ...
  out <- list(mu=MU, alpha01=ALPHA01, alpha03=ALPHA03, alpha05=ALPHA05,  alpha06=ALPHA06, beta1=BETA1, beta3=BETA3, beta5=BETA5, beta6=BETA6, sigma=SIGMA)
  return(out)
}
inits1 <- MakeInits()
inits2 <- MakeInits()
...
NAME <- "All_Dynamic_Standard_and_EventStandardDynamicX.bug"
...
write.csv(as.data.frame(all.data), "figure5_04.csv", row.names=F)
